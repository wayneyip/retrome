...
void MainWindow::updateAvatar()
{
    ...
    for (int i=0; i < equippedItemCount; i++)
    {
        ...
        {
            // Store the item's BACK sprite
            equippedItem = lItem->getSpriteName();

            // Flag isLayered to true.
            // Later when the Item* is popped off,
            // push it back into the heap with the FRONT's priority.

            // This is necessary to have a single Item*
            // go through the heap twice,
            // with a different priority each time.
            isLayered = true;
        }
        ...
        if (isLayered)
        {
            // Here we add the item's FRONT version back to the heap.
            lItem->toggleLayer();
...
void MainWindow::paintSprite(QPixmap& avatar, std::string spriteName, Color* color)
{
    ...
    // If sprite is to have a color...
    // (i.e. not a no-color category like clothing)
    if (color)
    {
        std::vector<Color::shade> shades = color->getShades();
        for (unsigned int i=0; i < shades.size(); i++)
        {
            // Create QColor with RGB values
            QColor shade(shades[i][0], shades[i][1], shades[i][2]);

            // Set to sprite's color index
            // (start at 3; 1 = black; 2 = white)
